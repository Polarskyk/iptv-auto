...
RUN apk update && apk add --no-cache gcc musl-dev python3-dev libffi-dev zlib-dev jpeg-dev wget make pcre-dev openssl-dev libxml2-dev libxslt-dev cargo rust \
  && pip install pipenv \
...
RUN apk update && apk add --no-cache ffmpeg pcre libxml2 libxslt
